...
export const contentSchemas = {
  ...
  artItems: z
    .array(
      z
        .object({
          ...baseGalleryItem,
          width: z.number().finite().int().min(1).max(5000).optional(),
          row: z.number().finite().int().min(0).max(500).optional(),
        })
        .passthrough(),
    )
    .max(MAX_ITEMS),
  ...
  profileData: z
    .object({
      name: text(200),
      headline: text(500).optional().or(z.literal('')),
      avatar: imageRef,
        ...
        .array(
          z
            .object({
              ...
              description: text(1000).optional().or(z.literal('')),
            })
            .passthrough(),
        )
        .max(MAX_LINKS),
