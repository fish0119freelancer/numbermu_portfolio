...
export function wouldExceedSaveImageTotal(currentValue, addedDataUrl, replacedDataUrl = null) {
  const base = totalInlineImageBytes(currentValue);
  const added = decodedDataUrlBytes(addedDataUrl);
  const removed = replacedDataUrl ? decodedDataUrlBytes(replacedDataUrl) : 0;
  const next = base + added - removed;
  if (next > MAX_SAVE_IMAGE_BYTES) {
    return {
      ok: false,
      nextBytes: next,
      message: `加入這張圖後，這次變更的圖片總量約 ${(next / 1024 / 1024).toFixed(2)} MiB，超過 12 MiB 上限。請先按「套用變更」存檔，再繼續上傳。`,
    };
  }
  return { ok: true, nextBytes: next };
}
