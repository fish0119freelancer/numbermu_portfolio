...
function checkMagicBytes(mime, buf) {
  if (mime === 'image/png') {
    ...
    const hasSignature = expected.every((b, i) => buf[i] === b);
    const hasIhdr = buf.subarray(12, 16).toString('ascii') === 'IHDR';
    const width = buf.readUInt32BE(16);
    const height = buf.readUInt32BE(20);
    const hasIend =
      buf.readUInt32BE(buf.length - 12) === 0 &&
      buf.subarray(buf.length - 8, buf.length - 4).toString('ascii') === 'IEND';
    return hasSignature && hasIhdr && width > 0 && height > 0 && hasIend;
  }
  if (mime === 'image/jpeg') {
  ...
  if (mime === 'image/gif') {
    ...
    return (
      ...
      buf.readUInt16LE(6) > 0 &&
      buf.readUInt16LE(8) > 0 &&
  ...
  if (mime === 'image/webp') {
    ...
    const declaredLength = buf.readUInt32LE(4) + 8;
    return (
      isRiff &&
      isWebp &&
      ['VP8 ', 'VP8L', 'VP8X'].includes(chunk) &&
      declaredLength === buf.length
    );
  }
  return false;
}
...
export function extractImagesFromPayload(payload, options = {}) {
  ...
  function transform(value) {
    if (typeof value === 'string') {
      ...
      const created =
        options.inMemory === true ? true : !fs.existsSync(absolutePath);
